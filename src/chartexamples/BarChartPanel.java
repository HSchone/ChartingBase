...
public class BarChartPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < n; i++) {
			// draw bars
			if (y[i] > b) {
				myRectangle = new Rectangle2D.Double(borderRectangle.x + (1.5 * i + 0.5) * barWidth,
						yPhysicalToyUser(borderRectangle, y[i], yMin, yMax), barWidth,
						clientBase - yPhysicalToyUser(borderRectangle, y[i], yMin, yMax));
			} else {
				myRectangle = new Rectangle2D.Double(borderRectangle.x + (1.5 * i + 0.5) * barWidth, clientBase,
						barWidth, yPhysicalToyUser(borderRectangle, y[i], yMin, yMax) - clientBase);
			}
			g2D.setPaint(c);
		...
		// line at base
		g2D.draw(new Line2D.Double(borderRectangle.x, clientBase, borderRectangle.x + borderRectangle.width - 1,
				clientBase));
		g2D.dispose();
	}
