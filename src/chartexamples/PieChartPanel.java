...
public class PieChartPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		// y - array of points (Double type) to chart (lower index is 1, upper
		// index is N)
		...
		for (int i = 0; i < n; i++) {
			myArc = new Arc2D.Double(borderRectangle.x, borderRectangle.y, borderRectangle.width,
					borderRectangle.height, startAngle, 360 * y[i] / sum, Arc2D.PIE);
			g2D.setPaint(c[i]);
			g2D.fill(myArc);
